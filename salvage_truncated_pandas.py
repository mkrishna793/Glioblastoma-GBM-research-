...
try:
    ...
    reader = pd.read_csv(os.path.join(data_dir, "variants.anno.csv.gz"), chunksize=50000)
    for chunk in reader:
        # Filter for coding mutations in current chunk
...
geno_chunks = []
try:
    reader = pd.read_csv(os.path.join(data_dir, "variants.passgeno.csv.gz"), chunksize=100000)
    for chunk in reader:
        # Filter for pass call = 1
        # Some genotype files might use 1.0 or 1, let's catch both
